...
class YolovClipReidentifier(BasePredictor, QObject):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None):
        ...
        self.args.classes = [2]  # 检测目标类型 0 行人 1自行车 2汽车
    ...
    def run(self):
        ...
        if not self.reid_model:
            self.reid_model = CLIPReIDModel('./clipreid/configs/VehicleID/vit_clipreid.yml',
                    ...
                    for r in self.results:
                        ...
                        for target_feat in target_feat_list:
                            for xyxy in indexs:
                                ...
                                # 相似度度量
                                #target_feat.cpu(), crop_feat.cpu()
                                current_similarity = self.reid_model.euclidean_distance_match(target_feat.cpu(), crop_feat.cpu())

                                print(current_similarity)
                                # 设置阈值
                                if current_similarity < 1.07:
