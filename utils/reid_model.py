...
class CLIPReIDModel:
    ...
    def __init__(self, config_dir, wight_dir):
        ...
        self.model = make_model(cfg,13164, 1, 1)
    ...
    #余弦相似度度量
    def cosine_similarity_match(self, query_feature, frame_feature):
        cosine_similarity = torch.nn.functional.cosine_similarity(query_feature, frame_feature)
        return cosine_similarity.item()

    #欧氏距离相似度度量
    def euclidean_distance_match(self, query_feature, frame_feature):
        query_feature = F.normalize(query_feature, p=2, dim=1)  # L2归一化
        frame_feature = F.normalize(frame_feature, p=2, dim=1)
        return torch.sqrt(torch.sum((query_feature - frame_feature) ** 2, dim=1))